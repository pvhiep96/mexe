  ...
  try {
    ...
    const orderData = {
      orders: {
        ...
        delivery_address:
          deliveryInfo.type === 'home' ? deliveryInfo.address : undefined,
        store_location:
          deliveryInfo.type === 'store' ? deliveryInfo.store : undefined,
        notes: note,
    ...
    try {
      ...
      if (!testResponse.ok) {
        ...
        throw new Error(
          `Backend không khả dụng: ${testResponse.status} ${testResponse.statusText}`
        );
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Failed to create order in backend: ${response.status}`
        );
      }
      ...
      if (fetchError instanceof Error) {
        if (fetchError.name === 'TimeoutError') {
          throw new Error(
            'Request timeout: Backend API không phản hồi trong thời gian quy định'
          );
        }
        throw fetchError;
      }
